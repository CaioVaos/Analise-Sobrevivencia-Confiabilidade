...
# Análise de Sobrevivência: estuda o tempo até a ocorrência de um evento de interesse, normalmente em indivíduos/pacientes.
#
# Confiabilidade: estuda o tempo até a ocorrência de uma falha em componentes, sistemas ou equipamentos.
#
# Semelhanças:
...
#
# Diferença principal:
# - na Sobrevivência, o evento geralmente está relacionado a indivíduos, por exemplo morte ou recaída;
# - na Confiabilidade, o evento geralmente é uma falha de um equipamento, componente ou sistema.

# Q2 ----

# As técnicas usuais não lidam adequadamente com censuras.
# Quando existe censura, não observamos o verdadeiro tempo do evento para todos os indivíduos.
#
# Por exemplo:
# Se um paciente ainda estiver vivo no fim do estudo, sabemos apenas que seu tempo de vida é MAIOR que o tempo observado.
# Ignorar isso e aplicar técnicas usuais trataria esse tempo censurado como se fosse um tempo completo, produzindo estimativas viesadas.

# Q3 ----

# Censura ocorre quando o evento de interesse não é observado para determinado indivíduo, 
# mas sabemos alguma informação parcial sobre seu tempo até o evento.
#
# O dado censurado não pode simplesmente ser removido porque ele ainda fornece informação sobre a sobrevivência.
# Um indivíduo censurado em t contribuiu para o estudo enquanto esteve sob risco, até o instante t.
...
#
# O pesquisador só observa os indivíduos e registra as informações, sem alterar a evolução do estudo.

## b) ----
#
# Estudos Experimentais
#
# O pesquisador intervém na evolução do extudo, experimentando tratamento no indivíduo.
...
#
# Descritivo:
# Apenas descreve características de uma população, sem necessariamente investigar relações causais.
#
# Caso-controle:
# Parte-se dos indivíduos com e sem o desfecho e investiga-se retrospectivamente a exposição.
#
# Coorte:
# Acompanha-se um grupo de indivíduos ao longo do tempo, observando a ocorrência do evento.
#
# Clínico aleatorizado:
# Indivíduos são aleatoriamente distribuídos entre tratamentos e posteriormente acompanhados para comparação dos desfechos.

# Q6 ----

## a) ----
#
# T = tempo, em semanas, desde a exposição ao material cancerígeno até o desenvolvimento do tumor de determinado tamanho.
...
## c) ----
#
# Rato D: censurado em 20 semanas, pois morreu acidentalmente antes de desenvolver o tumor de interesse.
#
...
#
# III. Evento de interesse: acontecimento cujo tempo até sua ocorrência está sendo estudado.

# Q8 ----
#
# Censura à Direita: sabemos que o evento não ocorreu até determinado instante, isto é, T > t.
# Exemplo: paciente termina o estudo sem morrer.
#
# Censura à e]Esquerda: sabemos que o evento já ocorreu antes de determinado instante, isto é, T < t.
# Exemplo: no primeiro acompanhamento o paciente já apresentava o sintoma.
#
# Censura Intervalar: sabemos apenas que o evento ocorreu entre dois instantes, isto é, a < T <= b.
...
# Q10 ----
#
# Não Informativa: o mecanismo de censura é independente do tempo de ocorrência do evento, condicionado às informações observadas.
# Exemplo: estudo termina em uma data pré-determinada para todos.
#
# Censura informativa: a probabilidade de censura está relacionada ao tempo de ocorrência do evento.
# Exemplo: pacientes com maior risco de morte abandonam o estudo com maior frequência por causa da própria condição clínica.
...
## a) ----
#
# Truncamento ocorre quando a própria inclusão do indivíduo na amostra depende do seu tempo de ocorrência do evento.
# Assim, certos indivíduos não aparecem na base.

## b) ----
#
# Truncamento à esquerda:
# indivíduos somente entram na amostra depois de determinado instante/critério inicial,
# fazendo com que parte do histórico anterior não seja observada.
#
...
## c) ----
#
# Porque os indivíduos que não apresentaram o evento antes do limite simplesmente não entram na amostra.
# Eles não aparecem como censurados; são excluídos pelo próprio mecanismo de seleção.
...
# Representa a taxa instantânea de ocorrência do evento no tempo t, condicionada a o indivíduo ter sobrevivido até t.
...
# vmr(t) = E(T-t | T>t).
# É o tempo médio que ainda resta após t entre os indivíduos que chegaram vivos/sem falha até t.
...
# Assim, entre os indivíduos que ainda não desenvolveram o tumor, a taxa instantânea de
# desenvolvimento do tumor é maior em tempos mais avançados.
...
## Q22 ----
#
# O estimador de Kaplan-Meier constrói a função de sobrevivência de forma acumulativa
# Calcula-se, em cada tempo de ocorrência do evento, a proporção de indivíduos que permanecem sem o evento entre aqueles que ainda estavam sob risco.
# As censuras não são consideradas como eventos, elas apenas indicam que o indivíduo deixa de ser acompanhado a partir daquele momento.
# Assim, o estimador utiliza as informações disponíveis em cada instante para atualizar progressivamente a estimativa de sobrevivência.

# Q23 ----
#
# Os intervalos de confiança precisam respeitar a natureza da função de sobrevivência, que deve estar entre 0 e 1.
# A transformação log-log é frequentemente conveniente pois coloca em temos do intervalo (0,1) sem resvalar em valores impossiveis.
...
#
# A transformação log é apropriada para intervalos de confiança,
# pois mantém os limites positivos, já que o risco acumulado não
# pode ser negativo.

## Q25 ----
#
# Percentis são obtidos identificando, na curva de sobrevivência, o tempo correspondente à probabilidade de sobrevivência desejada.
# A mediana, por exemplo, é o tempo em que S(t)=0,5, indicando que metade da população já apresentou o evento.

## Q26 ----
#
# 1. estima-se a curva de sobrevivência;
# 2. constrói-se o IC da curva;
# 3. transforma-se a posição horizontal correspondente ao percentil desejado em limites de tempo;
# 4. os pontos de interseção com os limites inferior e superior da curva geram o intervalo de confiança para o percentil.
...
#
# Ele pode ser preferido à média tradicional porque não exige que todos os indivíduos apresentem o
# evento e continua sendo interpretável mesmo quando a distribuição tem forte censura ou média não estimável.
...
## Q30 ----
#
# No Log-Rank usual, os tempos de ocorrência recebem o mesmo peso na construção da estatística.
# Em alguns problemas, as curvas podem diferir principalmente no início do acompanhamento.
# Nesse caso, testes ponderados podem atribuir maior peso aos tempos iniciais, quando há mais indivíduos sob risco.
#
...
## Q31 ----
#
# Ao comparar mais de dois grupos, vários testes pareados aumentam o número de hipóteses testadas, inflacionando o erro do tipo I.
# Por isso, são necessárias correções para múltiplas comparações.

## Q32 ----
#
# Bonferroni: controla o erro familiar dividindo alfa pelo número de comparações,mas é conservador.
#
# Holm-Bonferroni: também controla o erro familiar, mas ordena o p-valor, sendo menos conservador.
